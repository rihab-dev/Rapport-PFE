...
\begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth,keepaspectratio]{Images/authentification.png}
...
\subsection{Diagramme de compositions}
...
Dans le cadre de ce projet, nous avons choisi une stratégie de tests structurée, notre objectif est de vérifier le bon fonctionnement de l'application à plusieurs niveaux, tout en gardant un équilibre entre la couverture des fonctionnalités et le temps d'exécution des tests.
\newline La pyramide de tests se compose de trois niveaux :
...
\paragraph{Unitaire - Changer mot de passe}
Par ces tests, nous visons à valider le bon fonctionnement des méthodes \textbf{demanderReinitialisationMotDePasse()} et \textbf{reinitialiserMotDePasse()}, qui gèrent le processus de réinitialisation du mot de passe utilisateur.
...
\paragraph{Unitaire - Gestion client}
Par ces tests, nous visons à valider le bon fonctionnement des méthodes de gestion des clients, notamment \textbf{mettreAJourClient()}, \textbf{desactiverClient()}, \textbf{reactiverClient()}, \textbf{telechargerLogo()} et \textbf{supprimerLogo()}.

En utilisant Vitest, nous avons isolé ces tests du reste de l'application afin d'assurer leur indépendance. L'objectif est de s'assurer que ces méthodes traitent correctement les validations, la gestion des fichiers, les changements de statut et les cas d'erreur liés à la gestion complète des clients.
\newline
...
\item[$\bullet$] Cas où un logo est téléchargé et traité, ou erreur si aucun fichier n'a été trouvé.
...
\paragraph{Unitaire - Gestion des rôles et permissions}
Par ces tests, nous visons à valider le bon fonctionnement des méthodes \textbf{creerPermission()}, \textbf{attribuerRoleUtilisateur()} et \textbf{obtenirRolesEtPermissions()}, qui gèrent le système de contrôle d'accès basé sur les rôles (RBAC).

En utilisant Vitest, nous avons isolé ces tests du reste de l'application afin d'assurer leur indépendance. L'objectif est de s'assurer que ces méthodes respectent les règles d'exclusivité des rôles, valident l'unicité des permissions, gèrent l'intégrité des références utilisateur et traitent correctement les cas d'erreur.
\newline
Nous avons couvert les cas de test suivants :
\begin{itemize}

\item[$\bullet$] Cas où une permission est créée quand le code de permission est unique, et une exception est levée si la permission existe déjà.
\item[$\bullet$] Cas où un rôle est attribué à un utilisateur correctement, ou une exception est levée si l'utilisateur n'existe pas.
\item[$\bullet$] Cas où la règle d'exclusivité des rôles est respectée (CLIENT ne peut pas avoir CHEF\_PROJET ou DEVELOPPEUR), sinon une exception est levée.
\item[$\bullet$] Cas où l'attribution d'un rôle déjà assigné à un utilisateur échoue avec une exception appropriée.
\item[$\bullet$] Cas où la liste des rôles et permissions d'un utilisateur est retournée sans doublons.
\end{itemize}
\begin{figure}[H]
        \centering
...
Par ce test, nous visons à valider le bon fonctionnement de la méthode \textbf{modifierProfil()} exposée via l'endpoint \textbf{PUT /api/profil}, qui permet à un utilisateur de mettre à jour les informations de son profil.
...
\paragraph{E2E - Gestion des utilisateurs}
Par ces tests, nous visons à valider le bon fonctionnement du processus de désactivation et réactivation d'utilisateurs. En utilisant Playwright, nous avons testé les parcours complets de gestion du statut utilisateur en simulant les actions réelles d'un administrateur. L'objectif est de s'assurer que le système gère correctement les changements de statut et applique les restrictions d'accès appropriées.       
Nous avons couvert le cas de test suivant :
...
\paragraph{E2E - Authentification}
Par ces tests, nous visons à valider le bon fonctionnement du processus de connexion, ainsi que l'intégration des fournisseurs OAuth. En utilisant Playwright, nous avons testé les parcours complets de connexion en simulant les actions réelles des visiteurs. L'objectif est de s'assurer que le système valide les données d'authentification, gère les erreurs correctement et supporte l'authentification via Google et GitHub.
Nous avons couvert les cas de test suivants :
