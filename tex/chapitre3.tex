...
\hline
\textbf{Préconditions} & L'utilisateur accède à la page de connexion. Si la connexion est effectuée via le formulaire classique, il doit disposer d'un compte existant et actif. \\
\hline
\textbf{Postconditions} & L'utilisateur est authentifié. Un jeton d'accès est généré et un jeton de rafraîchissement est créé et persisté en base de données. L'utilisateur est redirigé vers l'interface correspondant à son rôle. \\
\hline
...
\item \textbf{Connexion OAuth Google/GitHub} : L'utilisateur choisit l'onglet \textit{Inscription OAuth}, sélectionne son rôle souhaité, puis lance l'authentification via Google ou GitHub. Si le compte OAuth existe déjà dans le système, l'utilisateur est authentifié et redirigé vers son tableau de bord.
\item \textbf{Compte OAuth introuvable} : Si aucun compte n'est trouvé lors d'une tentative de connexion OAuth, le système redirige l'utilisateur vers l'onglet \textit{Inscription OAuth} et affiche un message indiquant qu'aucun compte n'a été trouvé et qu'il peut faire une demande d'inscription.
\item \textbf{Compte en attente de validation} : Si une demande d'inscription OAuth est déjà en attente, le système affiche un message d'attente et informe l'utilisateur que son compte sera accessible après validation administrative.
\item \textbf{Changement de mot de passe obligatoire} : Si le compte exige une mise à jour du mot de passe, l'utilisateur est automatiquement redirigé vers une interface sécurisée pour modifier ses accès.
\end{enumerate} \\
\hline
\textbf{Scénarios d'exception} &
\begin{itemize}[nosep,leftmargin=0.6cm,topsep=0pt,itemsep=1pt, label=$\bullet$]
\item \textbf{Identifiants incorrects :} Le système affiche un message d'erreur générique et incrémente le compteur de tentatives échouées.
\item \textbf{Compte désactivé :} Le système informe l'utilisateur que son compte est désactivé et refuse l'accès.
\item \textbf{Verrouillage automatique :} Après \textbf{3 tentatives échouées}, le compte est bloqué pendant \textbf{1 minute}.
\item \textbf{Session expirée :} Si le jeton de rafraîchissement est invalide ou révoqué, l'utilisateur est redirigé vers la page de connexion.
\end{itemize} \\
\hline
\end{longtable}
...
\end{itemize} \\
\hline
\textbf{Scénario alternatif} & Le système génère un mot de passe temporaire. \\
\hline
\end{longtable}
\captionof{table}{Description textuelle « Ajouter utilisateur »} 
...
\end{minipage} \\
\hline
\textbf{Scénario alternatif} & L'administrateur réassigne les tickets ou les demandes. \\
\hline
\textbf{Scénarios d'exception} &
\begin{itemize}[nosep,leftmargin=0.6cm,topsep=0pt,itemsep=1pt, label=$\bullet$]
...
\hline
\textbf{Scénario nominal pour Modifier le profil d'un utilisateur} &
\begin{minipage}[t]{\linewidth}
\begin{enumerate}[label=\arabic*., leftmargin=0.6cm, nosep, itemsep=1pt]
...
\end{minipage} \\
\hline
\textbf{Scénario nominal pour Modifier son propre profil} &
\begin{minipage}[t]{\linewidth}
\begin{enumerate}[label=\arabic*., leftmargin=0.6cm, nosep, itemsep=1pt]
...
\hline
\textbf{Scénario alternatif} & L'administrateur restaure un rôle archivé. \\
\hline
\end{longtable}
...
\end{minipage} \\
\hline
\textbf{Scénario alternatif} & L'administrateur peut aussi attribuer le rôle depuis les détails du rôle en sélectionnant un ou plusieurs utilisateurs à qui l'attribuer. \\
\hline
\textbf{Scénarios d'exception} &
\begin{itemize}[nosep,leftmargin=0.6cm,topsep=0pt,itemsep=1pt, label=$\bullet$]
...
\end{minipage} \\
\hline
\textbf{Scénario alternatif} & L'administrateur utilise la matrice des permissions pour cocher les cases correspondant aux permissions à attribuer pour plusieurs rôles simultanément. \\
\hline
