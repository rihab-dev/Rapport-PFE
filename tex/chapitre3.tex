...
\begin{figure}[htbp]
        \centering
        \includegraphics[width=0.85\textwidth,height=0.75\textheight,keepaspectratio]{Images/Diagramme_cas_utilisation_sprint1.png}
...
\hline
\textbf{Postconditions} & L'utilisateur est authentifié. Un jeton d'accès est généré et un jeton de rafraîchissement est créé et persisté en base de données. \\
\hline
...
\item Le système récupère le type de profil (Client ou Employé) ainsi que les rôles et permissions associés.
...
\item \textbf{OAuth Google/GitHub :} L'utilisateur s'authentifie via un fournisseur tiers. Si le compte existe, il est connecté ; sinon, il est redirigé vers une finalisation d'inscription.
\item \textbf{Changement de mot de passe obligatoire :} Dans le cas où un renouvellement du mot de passe est exigé, l'utilisateur est automatiquement redirigé vers une interface sécurisée pour mettre à jour ses accès.
\end{itemize} \\
\hline
...
\item \textbf{Compte désactivé :} Le système informe l'utilisateur que son accès est suspendu.
