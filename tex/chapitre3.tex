...
\textbf{Utilisateur} (idUtilisateur, nom, prenom, email, motDePasse, estActif, \textbf{\#idRole})
\newline
\textbf{ProfilClient} (idProfilC, \textbf{\#idUtilisateur}, nomSociete, secteurActivite)
\newline
\textbf{ProfilEmploye} (idProfilE, \textbf{\#idUtilisateur}, matricule, poste)
\newline
\textbf{Projet} (idProjet, nomProjet, statut, \textbf{\#idProfilC}, \textbf{\#idChefProjet})
\newline
\textbf{MembreProjet} (\textbf{\#idProjet}, \textbf{\#idProfilE}, dateAffectation)
\newline
\textbf{DemandeOAuth} (idDemande, email, fournisseur, statut)
\newpage
...
\begin{itemize}
 ...
 \newline
  \noindent Cette suite de tests vise à valider le bon fonctionnement de la méthode \textbf{login()}, qui gère l'authentification classique des utilisateurs avec validation des identifiants, génération des jetons et gestion des tentatives échouées.
...
\item[$\square$]  \textbf{ Changer mot de passe}
\newline 
\noindent Cette suite de tests vise à valider le bon fonctionnement des méthodes \textbf{demanderReinitialisationMotDePasse()} et \textbf{reinitialiserMotDePasse()}, qui gèrent le processus de réinitialisation du mot de passe utilisateur.
...
\item[$\square$]  \textbf{Demande d'accès OAuth}
\newline 
\noindent Cette suite de tests vise à valider le bon fonctionnement de la méthode \textbf{handleOAuthRegistrationRequest()}, qui gère la création et la mise à jour des demandes d'inscription via OAuth, ainsi que la détection des doublons et la gestion des erreurs.
...
\item[$\square$]  \textbf{Gestion client}
\newline 
\noindent Cette suite de tests vise à valider le bon fonctionnement des méthodes de gestion des clients, notamment \textbf{mettreAJourClient()}, \textbf{desactiverClient()}, \textbf{reactiverClient()}, \textbf{telechargerLogo()} et \textbf{supprimerLogo()}.
...
\item[$\square$]  \textbf{Gestion des rôles et permissions}
\newline 
\noindent Cette suite de tests vise à valider le bon fonctionnement des méthodes \textbf{creerPermission()}, \textbf{attribuerRoleUtilisateur()} et \textbf{obtenirRolesEtPermissions()}, qui gèrent le système de contrôle d'accès basé sur les rôles (RBAC).
...
\begin{itemize}
  \item[$\square$] \textbf{Authentification (endpoint POST /api/auth/login)}
 \newline
  \noindent Cette suite de tests vise à valider le bon fonctionnement de l'endpoint \textbf{POST /api/auth/login}, qui traite l'authentification des utilisateurs et retourne les informations complètes incluant les jetons, rôles et permissions.
...
\begin{figure}[H]
        \centering
        \includegraphics[width=0.6\textwidth,keepaspectratio]{Images/test-interg-login.png}
        \caption{Test d'intégration : endpoint POST /api/auth/login}
    \end{figure}
\item[$\square$]  \textbf {Gestion du profil (endpoint PUT /api/profil)}
\newline 
...
\begin{itemize}
  \item[$\square$]  \textbf{Authentification}
 \newline
  \noindent Cette suite de tests vise à valider le bon fonctionnement du processus de connexion, ainsi que l'intégration des fournisseurs OAuth. En utilisant Playwright, nous avons testé les parcours complets de connexion en simulant les actions réelles des visiteurs. L'objectif est de s'assurer que le système valide les données d'authentification, gère les erreurs correctement et supporte l'authentification via Google et GitHub.
...
\newline 
\noindent Cette suite de tests vise à valider le bon fonctionnement du processus de désactivation et réactivation d'utilisateurs. En utilisant Playwright, nous avons testé les parcours complets de gestion du statut utilisateur en simulant les actions réelles d'un administrateur. L'objectif est de s'assurer que le système gère correctement les changements de statut et applique les restrictions d'accès appropriées.
