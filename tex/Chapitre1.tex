...
\begin{tikzpicture}
  \node[
    ...
    \textbf{\color{primary}Note sur les coûts}
    \par\vspace{4pt}
    Les tarifs annuels affichés pour 10 utilisateurs sont calculés selon la formule : (Prix mensuel par utilisateur) \(\times\) 10 utilisateurs \(\times\) 12 mois. GLPI représente le coût du support entreprise annuel (version communautaire gratuite). Freshdesk : 15€/mois \(\times\) 10 \(\times\) 12 = 1800€/an. Jira SM : 20€/mois \(\times\) 10 \(\times\) 12 = 2400€/an. Ces montants correspondent aux offres de base et peuvent augmenter avec des fonctionnalités additionnelles, supports premium ou services de personnalisation.
  };
\end{tikzpicture}
\end{center}
...
\item \textbf{Accepter le changement plutôt que suivre un plan immuable} : nous voyons le changement comme une opportunité d'adaptation. \newline Le projet doit pouvoir évoluer sans sacrifier la cohérence globale ni la qualité des livraisons. \cite{agile-manifesto}
\end{itemize}
...
Le Product Owner est responsable de la valeur métier du produit et de la vision métier du projet. Il définit les exigences de l’application et priorise les éléments \newpage du Product Backlog en fonction des objectifs de la société.
