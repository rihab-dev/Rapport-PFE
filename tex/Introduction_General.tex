\thispagestyle{empty}
\phantomsection
\addcontentsline{toc}{chapter}{Introduction générale}
\mtcaddchapter
\markboth{Introduction générale}{Introduction générale}

% Titre dans un cadre centré, sur la même page
\begin{center}
\vspace*{0.2cm}
\begin{tikzpicture}
  \node[
    draw        = primary,
    line width  = 0.8pt,
    fill        = white,
    inner xsep  = 2.0cm,
    inner ysep  = 0.5cm,
    text width  = 10cm,
    align       = center
  ]{
    {\fontsize{16}{20}\selectfont\bfseries\scshape\rmfamily\color{primary}
    Introduction générale}
  };
\end{tikzpicture}
\vspace{0.3cm}
\end{center}

\justifying
Dans un environnement technologique en constante évolution, la maintenance des systèmes informatiques représente un enjeu stratégique majeur pour les entreprises de services numériques.

Les organisations doivent concilier la réactivité face aux incidents, le respect des accords de niveau de service SLA (\textit{Service Level Agreement}) et la traçabilité des interventions.
\newline

\textbf{SolidWall Consulting}, acteur reconnu dans le domaine du développement web et mobile, n'échappe pas à cette réalité. En tant que prestataire de services numériques gérant un portefeuille croissant de clients et de contrats de maintenance, l'entreprise se trouve confrontée à des exigences de plus en plus élevées en matière de réactivité, de traçabilité et de respect des engagements de service. La maîtrise de ces enjeux constitue pour elle un facteur clé de compétitivité et de fidélisation de sa clientèle.
\newline

C’est dans ce contexte que s’inscrit notre projet de fin d’études, visant à concevoir et réaliser \textbf{SolidMaint}, une plateforme web B2B (\textit{Business-to-Business}) qui permet à SolidWall Consulting de gérer les relations contractuelles directement avec ses entreprises clientes. L’objectif principal consiste à automatiser et centraliser l’ensemble du processus métier, depuis la soumission des demandes clients jusqu’à la résolution des incidents, tout en garantissant une traçabilité complète et le respect des engagements contractuels.
\newline

Ce présent rapport s'articule autour de six chapitres retraçant la démarche complète du projet :

Le \textbf{premier chapitre} présente l'organisme d'accueil, l'analyse de l'existant, l'étude comparative des solutions du marché et le choix de la méthodologie Scrum.

Le \textbf{deuxième chapitre} porte sur la spécification des besoins, l'identification des acteurs, la modélisation UML (\textit{Unified Modeling Language}), la constitution du Product Backlog et l'architecture technique de l'application.

Le \textbf{troisième chapitre} couvre le premier sprint, dédié à l'authentification, la gestion des utilisateurs et le contrôle d'accès basé sur les rôles (RBAC).

Le \textbf{quatrième chapitre} traite le deuxième sprint, relatif à la gestion des clients, des projets et des contrats de maintenance, avec les alertes SLA et la traçabilité des activités.

Les \textbf{chapitres suivants} présentent les sprints restants, couvrant les tickets de maintenance, la collaboration et les fonctionnalités d'intelligence artificielle. Chaque chapitre inclut les tests et validations propres au sprint traité.



