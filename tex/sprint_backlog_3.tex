...
\multicolumn{4}{|l|}{\cellcolor{blue!20}\textbf{Epic 7 - Gestion des demandes clients}} \\
\hline
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint recueillant les réponses du client.
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint avec obligation de renseigner un motif.
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint pour centraliser le fil de discussion.
...
\multicolumn{4}{|l|}{\cellcolor{blue!20}\textbf{Epic 8 - Gestion des tickets}} \\
\hline
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint ségréguant les tickets par colonnes d'état.
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint calculant un score d'urgence composite par ticket.
\item Agréger les quatre composantes du score (priorité TypeSLA, \% SLA écoulé, utilisateurs impactés, tickets bloqués dépendants) et trier les résultats par ordre décroissant.
\item Restituer la file dans l'interface Next.js avec indicateur visuel par palier de criticité et détail du score en survol.
\end{itemize} & 2 \\
\hline
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint pour instancier une session d'activité active.
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint lié à un référentiel de motifs de suspension.
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint consolidant la session de travail.
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint permettant la saisie a posteriori d'un volume horaire.
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint contrôlant les règles de transition d'états.
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint actant la fin de l'intervention.
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint suite à une réclamation.
...
US59 & En tant que \textbf{utilisateur}, je souhaite commenter un ticket avec mentions afin de communiquer efficacement. &
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint prenant en charge l'identification ciblée d'utilisateurs.
...
US60 & En tant que \textbf{utilisateur}, je souhaite ajouter des pièces jointes aux tickets afin de fournir du contexte. &
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint.
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Mettre en oeuvre un service de synchronisation d'état en temps réel pour signaler la présence ou l'édition simultanée.
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Développer l'endpoint compilant l'ensemble des mutations de l'entité.
...
\multicolumn{4}{|l|}{\cellcolor{blue!20}\textbf{Epic 15 - Assistance intelligente (IA)}} \\
\hline
...
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
\item Mettre à disposition un bouton de génération sur la fiche ticket permettant de déclencher manuellement la suggestion IA.
\item Permettre au développeur de modifier ou compléter la suggestion avant publication sans aucune soumission automatique.
\end{itemize} & 2 \\
\hline
...
US101 & En tant que \textbf{chef de projet}, je souhaite que l'IA propose une classification des demandes clients afin d'accélérer leur traitement. &
\begin{itemize}[nosep, left=0pt, label={$\bullet$}]
